...
//       {loginResult && (
//         <div className={`message ${loginResult.type === 'success' ? 'success' : 'error'}`}>
//           <p3>{loginResult.message}</p3>  <button1 className="close-button" onClick={handleClose}>
//             &#x2716;
//           </button1>
//         </div>
//       )}
...
const Login = () => {
  ...
  const handleEmailSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/auth/check-email', {
      ...
      if (response.data.exists) {
        ...
        setLoginResult({ message: 'Email does not exist. Please try again.', type: 'error' });
      }
    } catch (error) {
      // Update state with the error message
      setLoginResult({ message: 'Error checking email. Please try again.', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
